...
def loss(logits, labels, nlabels, loss_type, weight_decay=0.0):
    ...
    if loss_type == 'weighted_crossentropy':
        ...
        segmentation_loss = (config.alfa)*losses.pixel_wise_cross_entropy_loss_weighted(logits, labels, class_weights=[0.076, 0.308, 0.308, 0.308]) + (1-config.alfa)*losses.focal_tversky_loss(logists, labels, only_foreground=True)  
    else:
